    ...
    return (
        <div className="bg-white w-full h-[calc(100vh-60px)] md:px-10 px-3 py-5 font-inter text-gray-600 font-medium overflow-y-auto border-t border-gray-100">
            ...
            <div className="flex flex-col gap-3 mt-5 border border-gray-100 md:p-5 px-2 py-5 rounded-xl">
                ...
                <div className="border-2 border-gray-200 w-full px-3 py-1 bg-gray-100 flex justify-between items-center rounded-md gap-2">
                    <input type="text" name="messageLink" id="messageLink" readOnly value={`whisperbin/send_anonymous/${username}`} className="md:text-[14px] text-[13px] outline-0 w-full" ref={to_copy} />
                    <button onClick={() => setText(to_copy.current.value)}><Copy size={16} /></button>
                </div>
                ...
                <div className="space-y-5">
                    {[{title : "Email Notification", label : "email"}, {title : "Push Notification", label : "push"}].map((item, i) => (
                        <div className="flex justify-between items-center" key={i}>
                            ...
                            <div className={`w-11 h-6 rounded-full flex items-center overflow-hidden ${preference[item.label] ? "bg-blue-500" : "bg-gray-300"}`} onClick={() => setPreferences(prev => ({...prev, [item.label]: !preference[item.label]}))}>
                                <div className={`bg-white w-5 h-5 rounded-full transform transition duration-500 ease-in-out ${preference[item.label] ? "translate-x-full" : "translate-x-0"}`} />
                            </div>
                        </div>
                    ))}
                </div>
