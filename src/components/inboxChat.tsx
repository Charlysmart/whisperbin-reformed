...
const InboxChat = ({ user, time, content, read } : InboxChatType) => {
    return (
        <div className="w-full md:h-[70px] h-[120px] px-2 bg-white flex items-center rounded-md gap-3 overflow-hidden">
            ...
            <div className="flex flex-wrap w-full justify-between px-2">
                <b className="md:w-[85%] w-[65%] md:text-[18px] text-[16px]">{user}</b>
                <p className="md:text-[12px] text-[12px]">{time}</p>
                <p className="md:text-[14px] text-[15px]">{content}</p>
            </div>
        </div>
    );
}
