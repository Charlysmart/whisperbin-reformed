...
export async function postData<T>({ url, data, onSuccess, onError, finallyCallback, navigate }: AxiosType<T>) {
  const path = window.location.pathname;
  try {
    const response = await axiosClient.post(url, data);
    if (onSuccess) onSuccess(response);   // Call success callback if provided
    return response;
  } catch (error: any) {
    ...
    if (finallyCallback) finallyCallback();  // Run finally callback if provided
  }
}
